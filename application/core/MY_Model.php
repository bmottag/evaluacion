...
class My_model extends CI_Model {
     ...
     */
    public function ejecutar_insert($tabla, $arrValores, $sentencia = 'no') {
        $this->sql = '';
        try {
            $this->sql = "INSERT INTO " . $tabla . " (" . implode(",", array_keys($arrValores)) . ") VALUES (";
            $i = 0;
            foreach ($arrValores as $campo => $valor) {
                $arrExp = array("nextval", "sysdate","to_char", "to_date", "to_timestamp");
    ...
    public function ejecutar_update($tabla, $arrValores, $arrWhere, $sentencia = 'no') {
        $this->sql = $sql = '';
        try {
